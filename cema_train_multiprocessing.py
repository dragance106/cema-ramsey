...
def train(compute_reward,
    ...
    obs_new = np.zeros([batch_size, autlen + 1, obslen], dtype=np.int8)
    # array of actions issued while constructing each graph in a generation
    act_new = np.zeros([batch_size, autlen], dtype=np.int8)
    ...
    for gen in range(num_generations):
        ...
        # just make sure that you stay within the limits if you came to the situation
        # that there are more than batch_size/2 triplets with almost the same maximum reward
        ind_survive = ind_survive[:batch_size//2]
